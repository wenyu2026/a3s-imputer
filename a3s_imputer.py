...
A3S  (Adaptive Ratio Imputer - Simple):     global profile, best for low missing rates.
LA3S (Local A3S):                           local k-NN profile, best for high missing rates.
Auto-A3S:                                   adaptive switch between A3S and LA3S.
...
class _BaseA3SImputer:
    """Base class with shared A3S logic."""
    ...
    def _estimate_global_profile(self, X_missing: np.ndarray) -> np.ndarray:
        ...
        if np.sum(complete_rows) >= 3:
            X_complete = X_missing[complete_rows][:, self.comp_idx]
            row_sums = X_complete.sum(axis=1, keepdims=True)
            row_sums = np.where(row_sums <= 0, self.min_positive, row_sums)
            X_norm = X_complete / row_sums
...
class A3SImputer(_BaseA3SImputer):
    """A3S: Adaptive Ratio Imputer - Global profile.
    ...
    def fit(self, X: np.ndarray) -> "A3SImputer":
        X = np.asarray(X, dtype=float)
...
# ============================================================================
# LA3S: Local Profile (k-NN)
# ============================================================================

class LA3SImputer(_BaseA3SImputer):
    """LA3S: Local A3S - k-NN local profile.
    ...
    def fit(self, X: np.ndarray) -> "LA3SImputer":
        X = np.asarray(X, dtype=float)
        self.global_profile_ = self._estimate_global_profile(X)
        X_filled = self._column_mean_init(X)
...
# ============================================================================
# Auto-A3S: Adaptive switch
# ============================================================================

class AutoA3SImputer:
    """Auto-A3S: Automatically select between A3S and LA3S based on missing rate.
    ...
    threshold : float, default=0.15
        Missing rate threshold. Below threshold -> A3S, above -> LA3S.
    n_neighbors : int, default=5
        Number of neighbors for LA3S (used only if selected).
    total : float, default=100.0
        Sum constraint.
    """
    ...
    def fit_transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if missing_rate < self.threshold:
            return a3s(X, comp_idx=self.comp_idx, non_comp_idx=self.non_comp_idx, total=self.total)
        else:
            return la3s(X, comp_idx=self.comp_idx, non_comp_idx=self.non_comp_idx, n_neighbors=self.n_neighbors, total=self.total)
...
def a3s(
    ...
    """A3S - low missing rate optimal (global profile)."""
    imputer = A3SImputer(comp_idx=comp_idx, non_comp_idx=non_comp_idx, total=total)
...
def la3s(
    ...
    """LA3S - high missing rate survival (local k-NN profile)."""
    imputer = LA3SImputer(comp_idx=comp_idx, non_comp_idx=non_comp_idx, n_neighbors=n_neighbors, total=total)
    return imputer.fit_transform(X)
...
def auto_a3s(
    ...
    """Auto-A3S - adaptive switch between A3S and LA3S."""
    imputer = AutoA3SImputer(comp_idx=comp_idx, non_comp_idx=non_comp_idx, threshold=threshold, total=total)
    return imputer.fit_transform(X)
